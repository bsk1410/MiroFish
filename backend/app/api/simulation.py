...
# Adding this prefix prevents Agent from calling tools, responding directly with text
...
def prepare_simulation():
    ...
    Features:
    - Auto-detects completed preparation work, avoids duplicate generation
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns simulation list with rich info like project name, description etc.
...
def start_simulation():
    ...
    About force parameter:
        ...
        - Suitable for scenarios needing to re-run the simulation
    ...
    About enable_graph_memory_update:
        ...
        - This lets the graph "remember" the simulation process, for subsequent analysis or AI dialogue
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (in command-waiting mode after completing simulation cycles)
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What is your overall view on this?",  // Required, interview question (same question for all Agents)
...
def get_interview_history():
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # When not specified, returns history from both platforms
        agent_id = data.get('agent_id')
...
def close_simulation_env():
    ...
    Send close environment command to simulation, gracefully exiting command-waiting mode.
    
    Note: This differs from /stop endpoint. /stop forcefully terminates the process,
