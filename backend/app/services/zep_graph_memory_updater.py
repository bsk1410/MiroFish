...
class AgentActivity:
    ...
    platform: str           # twitter / reddit platform
    agent_id: int
    agent_name: str
    ...
    def _describe_create_comment(self) -> str:
        """Create comment - includes comment content and target post info"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation action log files, updates new agent activities to Zep graph in real-time.
    Groups by platform, batch sends to Zep after accumulating BATCH_SIZE activities.
    
    All meaningful behaviors are updated to Zep, action_args contains full context info:
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        All meaningful behaviors are added to queue, including:
        - CREATE_POST (post)
        ...
        action_args contains full context info (e.g. post text, username, etc.).
