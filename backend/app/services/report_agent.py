...
2. First plan outline structure, then generate section by section
...
class ReportConsoleLogger:
    ...
    These logs differ from agent_log.jsonl and are plain text console output format.
    """
...
This is not LLM simulation, but calls real interview endpoints to get original answers from simulated Agents.
By default interviews on both Twitter and Reddit platforms simultaneously for more comprehensive perspectives.

Workflow:
1. Auto-reads profile files to understand all simulated Agents
...
# ── Report Outline Planning Prompt ──

PLAN_SYSTEM_PROMPT = """\
You are a "Future Prediction Report" writing expert, with a "God's perspective" on the simulated world - you can see every Agent's behavior, speech, and interaction.
...
- ✅ Agent behaviors in the simulated world are predictions of future population behaviors
...
The simulated world is a rehearsal of the future. We injected specific conditions (simulation requirements) into the simulated world. Agent behaviors and interactions in the simulation are predictions of future population behaviors.
...
2. [Must Quote Agent's Original Behaviors]
...
3. [Language Consistency - Quoted Content Must Match Report Language]
   ...
   - When quoting tool-returned content in a different language, translate it naturally into the report language
...
Platform A served as the primary scene for opinion, taking on the core function of initial information release:
...
- insight_forge: Deep insight analysis, auto-decomposes queries and retrieves facts and relationships multi-dimensionally
...
4. [Quote Format Standards - Must Be Standalone Paragraphs]
...
class ReportAgent:
    ...
    def _generate_section_react(
        ...
        conflict_retries = 0  # Consecutive conflict count of tool calls and Final Answer appearing together
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None, break loop for forced conclusion
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        "role": "user",
                        "content": (
                            "[Format Error] You included both tool calls and Final Answer in one reply, this is not allowed.\n"
                            "Each reply can only do one of the following:\n"
                            "- Call a tool (output one <tool_call> block, don't write Final Answer)\n"
                            "- Output final content (start with 'Final Answer:', don't include <tool_call>)\n"
                            "Please re-reply, only doing one of these."
                        ),
                    })
                    continue
                else:
...
class ReportManager:
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if this is a duplicate of the section title (skip duplicates in first 5 lines)
                if i < 5:
            ...
            # If previous line was a skipped title and current line is empty, skip it too
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        # Post-processing: clean title issues in the entire report
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if this is a duplicate title (same content title within 5 consecutive lines)
                is_duplicate = False
