...
Interface 2: Build knowledge graphs via Zep based on ontology definitions and text content
"""
...
class GraphBuilderService:
    """
    Graph Building Service
    Responsible for building knowledge graphs via Zep API
    """
    ...
    def build_graph_async(
        ...
        Args:
            ...
            batch_size: Number of chunks to send in each batch
    ...
    def _build_graph_worker(
        ...
        try:
            ...
            # 5. Wait for Zep to process chunks
            self.task_manager.update_task(
                task_id,
                progress=60,
                message="Waiting for Zep to process chunks..."
            )
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # This is required by Zep SDK and can be safely ignored
        ...
        for entity_def in ontology.get("entity_types", []):
            ...
            # Create attribute dictionary and type annotations (Pydantic v2 requires)
        ...
        for edge_def in ontology.get("edge_types", []):
            ...
            for attr_def in edge_def.get("attributes", []):
                ...
                # Zep API requires Field description, which is mandatory
                attrs[attr_name] = Field(description=attr_desc, default=None)
                annotations[attr_name] = Optional[str]  # Edge attribute type
    ...
    def add_text_batches(
        ...
        """Send text chunks in batches and return episode UUIDs"""
        episode_uuids = []
        total_chunks = len(chunks)
    ...
    def _wait_for_episodes(
        ...
        while pending_episodes:
            if time.time() - start_time > timeout:
                if progress_callback:
                    progress_callback(
                        f"Some episodes timed out, {completed_count}/{total_episodes} completed",
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing nodes and edges, including time information and detailed data
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)
